...
class FacetFieldProcessorByArrayMF extends FacetFieldProcessorByArray {
  ...
  boolean[] multiValuedFields;
  SortedSetDocValues[] dvIts;
  OrdinalMap[] ordinalMaps;
  int[] startTermIndices;
  int[] endTermIndices;

  FacetFieldProcessorByArrayMF(FacetContext fcontext, FacetField freq, SchemaField sf, SchemaField[] schemaFields) {
    super(fcontext, freq, sf);
    multiValuedField = sf.multiValued() || sf.getType().multiValuedFieldCache();
    this.schemaFields = schemaFields;
    int numFields = schemaFields.length;
    multiValuedFields = new boolean[numFields];
    for (int i = 0; i < numFields; i++) {
      multiValuedFields[i] = schemaFields[i].multiValued() || schemaFields[i].getType().multiValuedFieldCache();
    }
    startTermIndices = new int[numFields];
    endTermIndices = new int[numFields];
  }
  ...
  private void findStartAndEndOrdForField(int i) throws IOException {
    SchemaField currField = schemaFields[i];
    if (multiValuedFields[i]) {
      dvIts[i] = FieldUtil.getSortedSetDocValues(fcontext.qcontext, currField, null);
      if (dvIts[i] instanceof MultiDocValues.MultiSortedSetDocValues) {
        ordinalMaps[i] = ((MultiDocValues.MultiSortedSetDocValues) dvIts[i]).mapping;
      }
    } else {
      // multi-valued view
      SortedDocValues single = FieldUtil.getSortedDocValues(fcontext.qcontext, currField, null);
      dvIts[i] = DocValues.singleton(single);
      if (single instanceof MultiDocValues.MultiSortedDocValues) {
        ordinalMaps[i] = ((MultiDocValues.MultiSortedDocValues) single).mapping;
      }
    }

    if (dvIts[i].getValueCount() >= Integer.MAX_VALUE) {
      throw new SolrException(
              SolrException.ErrorCode.BAD_REQUEST,
              "Field has too many unique values. field=" + currField + " nterms= " + dvIts[i].getValueCount());
    }
